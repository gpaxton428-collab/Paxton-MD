...
export default {
  ...
  async execute(sock, msg, args, prefix) {
    ...
    if (choice !== 'on' && choice !== 'off') {
      ...
      return replyWithActions(sock, msg, `ℹ️ Anti-fake is currently *${settings.antifake ? 'ON' : 'OFF'}*.\nAllowed codes: ${settings.allowedCountryCodes?.length ? settings.allowedCountryCodes.join(', ') : 'none set — use .allowedcodes add <code>'}\nUsage: .antifake on|off`, antiActions(prefix, 'antifake'));
    }
    ...
    await replyWithActions(sock, msg, `✅ Anti-fake turned *${choice.toUpperCase()}*.`, antiActions(prefix, 'antifake'));
  }
};
