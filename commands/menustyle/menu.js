...
export default {
  ...
  async execute(sock, msg, args, currentPrefix, ctx) {
    const { BOT_NAME, BOT_MODE, VERSION, OWNER_NUMBER, isPrefixless, commandCategories, os, getGlobalSettings, getTotalCommandCount } = ctx;
    ...
    const footer = menuSettings.menuFooter || 'ᴘᴏᴡᴇʀᴇᴅ ʙʏ Paxton-Tech';
    let helpText = '';

    if (style === 2) {
      // Style 2 — clean minimal list, no boxes
      helpText += `⚡ *${BOT_NAME}*\n`;
      helpText += `Owner: ${getGlobalSettings().ownerName || 'Paxton'}  •  Prefix: ${isPrefixless ? 'none' : currentPrefix}  •  Commands: ${totalCommands}\n\n`;
      ...
      helpText += `╭─⌈ 🤖 *${BOT_NAME}* ⌋\n`;
      helpText += `│ 👑 Owner: ${getGlobalSettings().ownerName || 'Paxton'}\n`;
      ...
      helpText += `   ⚡ *${BOT_NAME}* ⚡\n`;
      helpText += `◈━━━━━━━━━━━━━━━◈\n`;
      helpText += `👑 Owner: ${getGlobalSettings().ownerName || 'Paxton'}   🟢 Online   📦 ${totalCommands} cmds\n\n`;
      for (const category of commandCategories.keys()) {
        const cmdList = commandCategories.get(category);
        helpText += `◆ *${category.toUpperCase()}*\n`;
        cmdList.forEach((cmd) => { helpText += `  ⟢ ${currentPrefix}${cmd}\n`; });
      ...
      const us = Math.floor(uptimeSec % 60);
      const now = new Date();
      const prefixLabel = isPrefixless ? 'none' : (ctx.getPrefixList?.() || [currentPrefix]).join(' ');
      helpText += `╭━━━〔 🌑 ${BOT_NAME.toUpperCase()} V${VERSION} 〕━━━┈⊷\n`;
      helpText += `┃ ${BOT_MODE === 'private' ? '🔒 Private' : '🌍 Public'}\n`;
      helpText += `┃ 💬 Prefix: ${prefixLabel}\n`;
      helpText += `┃ 👑 Owner: ${getGlobalSettings().ownerName || 'Paxton'} ⚡\n`;
      helpText += `┃ 📦 Plugins: ${totalCommands}\n`;
      helpText += `┃ 📊 Status: 🟢 ONLINE\n`;
      helpText += `┃ ⏱️ Runtime: ${uh}h ${um}m ${us}s\n`;
      helpText += `┃ 👤 User: @${(senderJid || '').split('@')[0]}\n`;
      helpText += `┃ 🕐 Time: ${now.toLocaleTimeString()}\n`;
      helpText += `┃ 📅 Date: ${now.toLocaleDateString()}\n`;
      helpText += `╰━━━━━━━━━━━━━━━┈⊷\n\n`;
      for (const category of commandCategories.keys()) {
        const cmdList = commandCategories.get(category);
        helpText += `╭━━━〔 ${category.toUpperCase()} 〕━━━┈⊷\n`;
    ...
    const menuContextInfo = { mentionedJid: [senderJid] };
