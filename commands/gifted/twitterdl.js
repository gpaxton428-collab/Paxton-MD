...
export default {
  name: 'twitterdl',
  alias: ['xdl'],
  description: 'Download a Twitter/X video. Usage: .twitterdl <url>',
  async execute(sock, msg, args) {
    const chatId = msg.key.remoteJid;
    const url = args[0];
    if (!url) return sock.sendMessage(chatId, { text: '❌ Usage: .twitterdl <url>' }, { quoted: msg });
    try {
      const data = await giftedJson('/download/twitter', { url: url });
