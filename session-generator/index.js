        ...
        if (connection === 'open') {
            (async () => {
                try {
                    ...
                    // These used to be fired-and-forgotten while a fixed 5s
                    // timer closed the socket underneath them — on anything
                    // but a fast connection the socket died mid-send and the
                    // DMs (session string / "Linked!" message) never arrived,
                    // even though the web page still showed the session fine
                    // (that's read from disk above, not from these sends).
                    // Now we actually wait for them, capped so a stuck send
                    // can't hold the job open forever.
                    ...
                    await withTimeout((async () => {
                        ...
                        try { await sock.sendMessage(sock.user.id, { text: job.sessionId }); } catch {}
                        try {
