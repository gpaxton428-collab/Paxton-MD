...
// Short TTL cache avoids calling groupMetadata repeatedly for every command
// in a busy group while still picking up admin/member changes quickly.
const GROUP_CACHE_TTL = 5000;
const groupCache = new Map();

export async function getGroupMetadata(sock, groupJid, { fresh = false } = {}) {
  if (!groupJid) return null;
  const cached = groupCache.get(groupJid);
  if (!fresh && cached && Date.now() - cached.time < GROUP_CACHE_TTL) return cached.value;
  try {
    const value = await sock.groupMetadata(groupJid);
    groupCache.set(groupJid, { value, time: Date.now() });
    return value;
  } catch {
...
export function invalidateGroupMetadata(groupJid) {
  if (groupJid) groupCache.delete(groupJid);
}
