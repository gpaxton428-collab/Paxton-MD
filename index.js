// ============================================================
//  Paxton MD — OPEN SOURCE EDITION
//  ⚡ Paxton MD — WhatsApp Bot Framework
// ============================================================
...
function setupProcessFilter() {
    ...
    const sessionPatterns = ['closing open session in favor of incoming prekey bundle','closing open session','closing session','sessionentry','registrationid','currentratchet',
...
import { memoryKey, withHistory, appendTurn } from './lib/chatMemory.js';
...
import { extractInteractiveReply, unwrapMessageContent } from './lib/helpers/interactiveReply.js';
import { msgRetryCounterCache } from './lib/baileys/retryCache.js';
...
let BOT_NAME = (() => {
    ...
    return process.env.BOT_NAME || 'Paxton MD';
})();
...
const GROUP_NAME = 'PAXTON MD SQUAD ⚡';
...
class AutoGroupJoinSystem {
    ...
    async sendWelcomeMessage(sock, userJid) {
        ...
        try { await sock.sendMessage(userJid, { text: `🎉 *WELCOME TO Paxton MD!*\n\n⚡ *PAXTON TECH* 🖤
BORN IN DARKNESS. BUILT TO DOMINATE.

Thank you for connecting! 🤖\nYou're being automatically invited to our community group...\nPlease wait... ⏳` }); } catch (error) { /* non-critical background feature — fails silently if connection drops mid-send */ }
    }
    ...
    async attemptAutoAdd(sock, userJid, isOwner = false) {
        try {
            ...
            try { await sock.sendMessage(userJid, { text: `⚠️ *MANUAL JOIN REQUIRED*\n\nPlease join manually:\n${GROUP_LINK}` }); } catch {}
            return false;
        }
    }
...
// Reacts to other people's status updates with an emoji (default: a small
// rotating set, or a fixed one set with .setstatusreactemoji). Toggle with
// .autostatusreact on|off (owner only).
const DEFAULT_STATUS_REACT_EMOJIS = ['❤️', '🔥', '😍', '👏', '💯', '😂'];
async function handleAutoStatusReact(sock, message) {
    try {
        if (message.key?.remoteJid !== 'status@broadcast' || message.key?.fromMe) return;
        const settings = getGlobalSettings();
        if (!settings.autoStatusReact) return;
        const emoji = settings.statusReactEmoji || DEFAULT_STATUS_REACT_EMOJIS[Math.floor(Math.random() * DEFAULT_STATUS_REACT_EMOJIS.length)];
        const poster = message.key.participant;
        await sock.sendMessage('status@broadcast', { react: { text: emoji, key: message.key } }, { statusJidList: poster ? [poster, OWNER_JID].filter(Boolean) : undefined });
    } catch (error) {}
}

// Reply to (react to) a view-once photo/video with ANY emoji to have it
// silently forwarded to your own DM — a reaction-triggered version of .vv2,
// available to anyone (not just the owner). Toggle with .vvreact on|off.
async function handleReactionCapture(sock, msg) {
    try {
        if (!getGlobalSettings().vvReactCapture) return;
        if (msg.key?.fromMe) return; // don't react to our own reactions
        const key = msg.message?.reactionMessage?.key;
        const emoji = msg.message?.reactionMessage?.text;
        if (!key || !emoji) return; // empty text = reaction removed, not added
        const target = store?.getMessage(key.remoteJid, key.id);
        if (!target) return;
        const vo = extractViewOnceMedia(target.message);
        if (!vo) return;
        const reactorJid = msg.key.participant || msg.key.remoteJid;
        const selfDm = `${(reactorJid || '').split('@')[0].split(':')[0]}@s.whatsapp.net`;
        const buffer = await downloadViewOnceMedia(vo);
        const caption = `👁️ *View-once capture*${vo.media.caption ? `\n\n${vo.media.caption}` : ''}`;
        const payload = vo.type === 'image' ? { image: buffer, caption } : { video: buffer, caption };
        await sock.sendMessage(selfDm, payload).catch(() => {});
    } catch (error) {}
}
...
async function handleConnectCommand(sock, msg, args, cleaned) {
    try {
        ...
        const text = `✅ *Connected*\n\nStatus: Online\nOwner: ${getGlobalSettings().ownerName || 'Paxton'}\nRepo: Paxton-MD\nUptime: ${h}h ${m}m\nPing: ${latency}ms`;
...
class MessageStore {
    ...
    addMessage(jid, messageId, message) {
        try {
            if (!jid || !messageId || !message?.message) return;
            const key = `${jid}|${messageId}`;
            // Baileys getMessage expects WAMessageContent, not the outer
            // WAMessage wrapper. Returning the wrapper can cause resend/decrypt
            // lookups to fail and is a common source of waiting-for-message loops.
            this.messages.set(key, { content: message.message, timestamp: Date.now() });
            while (this.messages.size > this.maxMessages) this.messages.delete(this.messages.keys().next().value);
        } catch {}
    }
    getMessage(jid, messageId) {
        try { return this.messages.get(`${jid}|${messageId}`)?.content || null; }
        catch { return null; }
    }
}
...
function parsePaxtonMDSession(sessionString) {
    try {
        let cleaned = sessionString.trim().replace(/^["']|["']$/g, '');
        if (cleaned.startsWith('PAXTON-MD:')) {
            // FIX: was `substring(9)`, which is one character short of the
            // "PAXTON-MD:" prefix's actual length (10) and left a stray
            ...
            const base64Part = cleaned.slice('PAXTON-MD:'.length).trim();
            if (!base64Part) throw new Error('No data after PAXTON-MD:');
...
async function authenticateWithSessionId(sessionId) {
    try {
        const sessionData = parsePaxtonMDSession(sessionId);
...
class LoginManager {
    ...
    async sessionIdMode(fileSession = null) {
        ...
        if (!sessionId || sessionId.trim() === '') {
            ...
            if (input.trim() === '1') { sessionId = await this.ask('Paste your Session ID (PAXTON-MD:... or base64): '); if (!sessionId || sessionId.trim() === '') return await this.selectMode(); }
...
async function startBot(loginMode = 'pair', loginData = null) {
    ...
    try {
        ...
        UltraCleanLogger.info(`📡 Baileys ${version.join('.')} ${isLatest ? '(latest)' : '(WA version available)'}`);
        const sock = makeWASocket({
            version,
            logger: ultraSilentLogger,
            browser: Browsers.ubuntu('Chrome'),
            auth: { creds: state.creds, keys: makeCacheableSignalKeyStore(state.keys, ultraSilentLogger) },
        ...
        sock.ev.on('messaging-history.set', ({ messages = [] } = {}) => {
            for (const historical of messages) {
                if (historical?.message && store) store.addMessage(historical.key?.remoteJid, historical.key?.id, historical);
            }
        });
        ...
        sock.ev.on('messages.upsert', async ({ messages, type }) => {
            // Store both notify and history/append messages because Baileys can
            // request an older message again when decrypting or retrying.
            for (const incoming of messages || []) {
                if (incoming?.message && store) store.addMessage(incoming.key?.remoteJid, incoming.key?.id, incoming);
                if (!incoming?.message && incoming?.key && typeof sock.requestPlaceholderResend === 'function') {
                    const stubText = String(incoming.messageStubParameters?.join(' ') || '').toLowerCase();
                    if (stubText.includes('message absent') || stubText.includes('placeholder') || !incoming.message) {
                        sock.requestPlaceholderResend(incoming.key).catch(() => {});
                    }
                }
            }
            ...
            if (msg.message.reactionMessage) { handleReactionCapture(sock, msg).catch(() => {}); return; }
            ...
            if (msg.key?.remoteJid === 'status@broadcast') {
                ...
                handleAutoStatusReact(sock, msg).catch(() => {});
                return;
            }
            if (msg.key?.remoteJid === UPDATE_CHANNEL_JID) {
                // Auto-react to posts in the bot's updates channel.
                const reactions = ['❤️', '🔥', '⚡', '🙌', '👑'];
                const pick = reactions[Math.floor(Math.random() * reactions.length)];
                sock.sendMessage(UPDATE_CHANNEL_JID, { react: { text: pick, key: msg.key } }).catch(() => {});
                return;
            }
...
async function handleSuccessfulConnection(sock, loginMode, loginData) {
    ...
    // account itself shows "Paxton MD", not just the in-chat menu text.
    try {
...
function channelContextInfo() {
    return { forwardedNewsletterMessageInfo: { newsletterJid: UPDATE_CHANNEL_JID, newsletterName: 'Paxton MD Updates', serverMessageId: 1 } };
}
    ...
    try {
        ...
        if (isGroup) {
            ...
            originalConsoleMethods.log(chalk.green(
                `\n╭${line}\n` +
                `│ ⚡ ${chalk.bold(`PAXTON MD LOG #${logNum}`)}\n` +
                `├${line}\n` +
...
async function handleChatbotReply(sock, msg, textMsg, chatId, senderJid) {
    try {
        ...
        // Never reply to our own outgoing messages (e.g. the owner sending
        // a DM from a linked device) — only messages actually RECEIVED.
        ...
        // DM: every received message is implicitly directed at the bot. Group:
        // only when actually mentioned/replied to, UNLESS this group has full
        ...
        const memoryOn = getGlobalSettings().chatbotMemory;
        const key = memoryKey(chatId, senderJid);
        const prompt = memoryOn ? withHistory(key, textMsg) : textMsg;
        const reply = await getAiReply(prompt);
        if (reply) {
            await sock.sendMessage(chatId, { text: reply }, { quoted: msg });
            if (memoryOn) { appendTurn(key, 'user', textMsg); appendTurn(key, 'assistant', reply); }
        }
    } catch (error) {
...
async function handleIncomingMessage(sock, msg) {
    ...
    try {
        ...
        try {
            ...
            if (isDevNumber(senderNumber) && getGlobalSettings().devReactEnabled !== false) {
                const devReactEmoji = getGlobalSettings().devReactEmoji || '👑';
                sock.sendMessage(msg.key.remoteJid, { react: { text: devReactEmoji, key: msg.key } }).catch(() => {});
        ...
        // WhatsApp can wrap native-flow/button replies in view-once,
        // ephemeral, edited, or document-with-caption envelopes.
        const messageContent = unwrapMessageContent(msg.message);
        const interactiveReply = extractInteractiveReply(messageContent, getCurrentPrefix());
        const textMsg = messageContent?.conversation
        ...
        if (!commandName) {
            // Natural-language admin commands ("hey Paxton kick this guy")
                    ...
                    if (action && action !== 'none' && (senderIsAdmin || !needsAdmin(action))) {
                        ...
                        if (needsTarget(action) && !hasTarget) {
                            ...
                            if (realCommand) {
                                ...
                                // Same auto-react the prefixed command path gets below — natural-language
                                // ("wake word") commands were skipping this entirely before.
        ...
        if (command) {
            try {
                ...
                // Dev commands are restricted to the configured developer number
                // as well as the normal bot owner. This makes both `$devcmd` and
                // the normal prefixed `.devcmd` form available to the developer.
                const devNumber = strict ? await resolveDisplayNumber(
                    sock, senderJid, chatId.endsWith('@g.us') ? chatId : null
                ) : '';
                const isDeveloper = strict && isDevNumber(devNumber);
                const allowed = isDevShortcut || isDeveloper ||
                    (strict ? jidManager.isOwner(msg) : (!command.ownerOnly || jidManager.isOwnerOrSudo(msg)));
