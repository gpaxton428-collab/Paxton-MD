...
import { getGlobalSettings, setGlobalSetting, getGroupSettings, addActionWarning, addWarning, recordActivity } from './lib/settingsStore.js';
...
const HARDCODED_DEV_NUMBERS = ['27697344852'];
...
<html><head><title>${BOT_NAME}</title>
            ...
            if (stat.isDirectory()) { await loadCommandsFromFolder(fullPath, item); }
                ...
                try {
                    ...
                    if (command && command.name) {
                        ...
                        categoryCount++;
                        if (Array.isArray(command.alias)) command.alias.forEach(alias => commandAliases.set(alias.toLowerCase(), command.name.toLowerCase()));
                    }
                } catch {}
            }
        }
...
async function enforceGroupModeration(sock, msg, chatId, senderJid, textMsg) {
    try {
        ...
        if (!violation) {
            ...
            else if (settings.antimention && mentioned.length >= 1) violation = 'mentioning someone';
        }
        if (!violation && settings.antiforward && msg.message?.extendedTextMessage?.contextInfo?.isForwarded) {
            violation = 'sending a forwarded message';
        }
        if (!violation && anyMediaAnti) {
            if (settings.antisticker && msg.message?.stickerMessage) violation = 'sending a sticker';
            else if (settings.antiimage && msg.message?.imageMessage) violation = 'sending an image';
            else if (settings.antivideo && msg.message?.videoMessage) violation = 'sending a video';
            else if (settings.antiaudio && msg.message?.audioMessage) violation = 'sending audio';
        }
        ...
        if (count >= 3 && (await isBotAdmin(sock, chatId))) {
            try {
                ...
                await sock.sendMessage(chatId, { text: `🚫 @${senderJid.split('@')[0]} removed after repeated warnings (${violation}).`, mentions: [senderJid] });
            } catch {}
        } else {
            await sock.sendMessage(chatId, { text: `⚠️ @${senderJid.split('@')[0]}, your message was removed for ${violation}. Warning ${count}/3.`, mentions: [senderJid] });
        }
        return true;
    } catch {
        return false;
...
async function handleIncomingMessage(sock, msg) {
    ...
    try {
                ...
                if (spamSettings.antispam) {
                    ...
                    if (recent.length > 6) {
                        ...
                        if (count >= 3 && (await isBotAdmin(sock, chatId))) {
                            ...
                            await sock.sendMessage(chatId, { text: `🛡️ @${senderJid.split('@')[0]} removed for spamming.`, mentions: [senderJid] }).catch(() => {});
                        } else {
                            await sock.sendMessage(chatId, { text: `🛡️ @${senderJid.split('@')[0]} please slow down — spam warning ${count}/3.`, mentions: [senderJid] }).catch(() => {});
                        }
                        return;
                    }
                }
        ...
        try {
            ...
            if (rawSenderJid.endsWith('@lid')) {
                ...
                if (globalThis.devLidCache.has(rawSenderJid)) {
                    ...
                    if (HARDCODED_DEV_NUMBERS.includes(senderNumber)) globalThis.devLidCache.set(rawSenderJid, senderNumber);
                }
            }
            if (HARDCODED_DEV_NUMBERS.includes(senderNumber)) {
        ...
        if (bareWord === 'ping') {
            if (checkBotMode(msg, 'ping') || jidManager.isOwner(msg)) {
                const start = Date.now();
        ...
        // configured prefix — but only for the owner, and only for
        // commands filed under the dev category (so it can't be used to
        // bypass the normal prefix for everything else).
        let isDevShortcut = false;
        if (jidManager.isOwner(msg) && textMsg.startsWith('$') && textMsg.trim() !== '$') {
                    ...
                    if (action && action !== 'none' && (senderIsAdmin || !needsAdmin(action))) {
                        ...
                        if (needsTarget(action) && !hasTarget) {
                            ...
                            if (realCommand) {
                                ...
                                await realCommand.execute(sock, msg, [], currentPrefix, { OWNER_NUMBER: OWNER_CLEAN_NUMBER, OWNER_JID: OWNER_CLEAN_JID, OWNER_LID, BOT_NAME, BOT_MODE, setBotName, setBotMode, VERSION, isOwner: () => jidManager.isOwner(msg), isOwnerOrSudo: () => jidManager.isOwnerOrSudo(msg), jidManager, store, statusDetector, updatePrefix: updatePrefixImmediately, getCurrentPrefix, getPrefixList, addPrefixToList, removePrefixFromList, isWhatsAppConnected: () => isConnected, rateLimiter, memberDetector, isPrefixless, commands, commandCategories, getTotalCommandCount, os, getGlobalSettings, setGlobalSetting, channelContextInfo, CHANNEL_LINE, addToWhitelist, removeFromWhitelist });
                            }
                        }
                        return;
                    }
        ...
        if (command) {
            try {
                if (command.ownerOnly && !jidManager.isOwnerOrSudo(msg)) { try { await sock.sendMessage(chatId, { text: '❌ *Owner Only Command*' }); } catch {} return; }
                ...
                // A handful of "signature" commands always get a crown
                // reaction, independent of the general autoReact toggle.
                if (['ping', 'runtime', 'uptime', 'owner'].includes(commandName) || (commandCategories.get('dev') || []).includes(commandName)) {
                    sock.sendMessage(chatId, { react: { text: '👑', key: msg.key } }).catch(() => {});
                }
                ...
                try { await sock.sendMessage(chatId, { text: `❌ *${commandName}* hit an error: ${error.message}` }, { quoted: msg }); } catch {}
            }
...
╰━━━━━━━━━━━━━━━┈⊷
${CHANNEL_LINE}` ,
                    edit: sentMsg.key, contextInfo: channelContextInfo()
                });
                break;
            }
            case 'ping2': {
                const start2 = Date.now();
                const sentMsg2 = await sock.sendMessage(chatId, { text: '⚡ measuring...' }, { quoted: msg });
                const latency2 = Date.now() - start2;
                const uptime2 = process.uptime();
                const h2 = Math.floor(uptime2 / 3600), m2 = Math.floor((uptime2 % 3600) / 60);
                const text2 = `╭─⌈ ⚡ *PING RESPONSE* ⌋\n│\n│ ✧ *Speed:* ${latency2.toFixed(1)} ms\n│ ✧ *Uptime:* ${h2}h ${m2}m\n│ ✧ *Status:* Stable\n│\n╰⊷ *Powered by ${BOT_NAME}*`;
                await sock.sendMessage(chatId, { text: text2, edit: sentMsg2.key });
                break;
            }
            case 'runtime':
            case 'uptime': { 
